...
class CommandInterpreter(input: InputStream, output: OutputStream) : Runnable, Observer {
    ...
    override fun newMessage(message: ChatMessage, privateMsg: Boolean) {
        when (message.username) {
            "Server" -> printer.println(
                "${message.username}: [${message.time.format(formatter)}] ${message.message}")
            "TopChatter" -> printer.println("${message.message}")
            else -> {
                when (privateMsg) {
                    false -> printer.println(
                            "${message.username}: [${message.time.format(formatter)}] ${message.message}")
                    true -> printer.println(
                            "private ${message.username}: [${message.time.format(formatter)}] ${message.message}")
                }
            }
        }
    }
                                ...
                                ":?" -> showHelp()
                                    ...
                                    when (userInput.substring(0, min(6, userInput.lastIndex))) {
                                        ":user " -> {
                                            if (userInput.substringAfter(' ').isNotBlank()) {
                                                ...
                                                if (Users.addUser(username)) { // addUser returns true is succeeds
                                                    ...
                                                    ChatHistory.notifyObservers(
                                                            ChatMessage("$username joined", "Server", LocalDateTime.now())
                                                    ...
                                                    username = ""
                                                    printer.println(" >> ${userInput.substringAfter(' ')} already exists.")
                                            ...
                                            printer.println(" >> User name is not set!")
                                            //printer.println(" >> Use \":?\" for help.")
                                        }
                                    }
                                }
                        ...
                        when (userInput[0]) {
                            ':' -> {
                                when (userInput.substringBefore(' ')) {
                                    ...
                                    ":topchatter" -> printer.println(TopChatter.showTopChatters())
                                    ":logout" -> {
                                        ChatHistory.logout(username, this) // removes (observer,username) pair from the map
                                        ChatHistory.notifyObservers(
                                                ChatMessage("$username Left", "Server", LocalDateTime.now())
                            ...
                            '@' -> { //private messages/whisper
                                ...
                                val publicReceiver = ChatHistory.observerListLoggedIn // send to everyone including me
